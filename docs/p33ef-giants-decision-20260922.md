...
> **实测修正（round 112，P3.3e 设计步）**：共享 helper 的数目与行数都比这里估计得多——**11 个** helper 在巨方法之外仍有读者、共 **142 行**；其中 4 个可**下沉到纯模块**（与层工作第 2 条同一手法），其余在「先下沉、再重算子调用」的**不动点**下会进一步减少。因此端口落在 **10～15** 之间，由搬移时的不动点测量定案（设计记录 docs/p33e-derivation-slice-design-20260922.md 第 2、4 节）。该设计记录的初版曾写「15 个 helper 全部随迁、端口 6 -> 8」，那是扫描看不见 self. 调用造成的**错误结论，已撤回**（同记录第 2 节与被撤回原文）。
