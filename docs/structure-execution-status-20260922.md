...
1. **P3.4-1** - move the four pure helpers (`_select_report_evidence_rows`, `_migrate_snapshot_payload`, `_canonical_sha256`, `_snapshot_report_context`) plus `_REPORT_PROJECTION_EVIDENCE_LIMIT` into `report/revision_writer.py` with the 11-member host pin and a TRACKED contract test; their pin is only the two `cls`-reached members (`_canonical_json_chunks`, `SNAPSHOT_SCHEMA_VERSION`).
2. **P3.4-2** - move the revision lifecycle (`_create_report_revision`, `edit_report`, `get_report_revision`, `recompose_report`, `publish_report`, `submit_analyst_draft`, `workbench_submit_analyst_draft`) and `ReportComposeGateRejected` (re-exported `X as X` from `service.py` for exception identity), then record the `report.revision_writer -> models` DECISION in `recorded_allowed_edges` + `docs/plan-conflict-resolutions-20260922.md`.
3. **HARD GATES FOR EVERY SLICE - a slice missing any of these is NOT complete** (plan §4 plus this design): 1. `py scripts/check-slice-tooling.py` (+ `--self-check`); 2. the tool's own can-fail proof against a damaged subject, restored byte-for-byte; 3. `python -m compileall`; 4. the scope gate (only `allowed_files` changed); 5. `py scripts/check-import-graph.py --strict --policy docs/import-policy.json`; 6. `docker compose config`/`ps`, `--strict --services`, BLOCKED classification, never skipping `ghidra-worker`; 7. focused tests -> full suite -> `scripts/structure_behavior_probe.py --check`; 8. failure-set comparison by NODE SET, never by count; 9. deployment gate after a rebuild, then `HEAD == origin/main`; 10. the TRACKED contract test asserts the pin in both directions; 11. a delegation-shape gate (every delegation forwards every signature parameter - this phase has been bitten twice).
