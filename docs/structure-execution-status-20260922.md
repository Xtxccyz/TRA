...
1. **FIRST MEASURE, THEN FIX, THE OPEN SPEAKEASY VIOLATION** (my previous cause was WRONG and is retracted: the fixture DOES supply a sample, so the plan branch runs). Run `python -m pytest -q -s tests/test_speakeasy_reachability.py -k granted_window_planning` with a recorder on the plan call and read the UNFILTERED output: either the recorder sees `allow_speakeasy=False` (the operator decision is not reaching the plan - a live defect) or the monkeypatch never bit (the function is not reachable as `service.controlled_emulation_windows`). Only then write the assertion, and require a can-fail that forces the plan's decision to False - scoped BY LINE, since that argument text occurs twice in `service.py`. The violation is OPEN; the file is at HEAD and green, and no wrong expectation has been shipped.
