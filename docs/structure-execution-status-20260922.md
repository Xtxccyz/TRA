...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 73 条，本文件只汇总。
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE; PHASE 3 IS IN PROGRESS and **P3.3 IS COMPLETE** (the derivation giant, the loop, the three layer items, the sink and every coordinator slice moved; `service.py` 29,640 -> 20,486 lines). NOW DESIGNED AND MEASURED: **P3.4, `ReportRevisionWriter`** - 11 members / 795 def-span lines (798 with decorators) plus `_REPORT_PROJECTION_EVIDENCE_LIMIT`, behind an 11-member host pin each with a recorded reason, in two steps (P3.4-1 pure helpers, P3.4-2 the revision lifecycle incl. `edit_report` and the compose-gate exception). AFTER P3.4: P3.5 (EmulationCoordinator), P3.6 (WorkbenchQueryReader), P3.7 (test surface), then Phases 4 and 5. P3.3b(2) and P3.3d(2) remain layer-blocked.
...
| phase_3 | IN PROGRESS - P3.1/P3.2/P3.3 COMPLETE (P3.3's slices, the three layer items, the sink, the derivation giant and the 3,523-line loop; service.py 29,640 -> 20,486). P3.4 is DESIGNED (ReportRevisionWriter: 11 members / 795 def-span lines, 11-member host pin, two steps, one accepted review hole `edit_report`, one accepted layer correction on the `models` edge). NEXT: P3.4-1, P3.4-2, then P3.5-P3.7; P3.3b(2)/P3.3d(2) remain layer-blocked. |
...
PASS - 128 files per service across 8 services, missing=0 differing=0 container-only=0, import smoke 56 enumerated modules per service, 11 containers running; gate output names HEAD a83126b6680f
...
1. **P3.4-0 (prerequisite, measured, not yet done): generalise `.scratch/p33-extract.py` off `investigation/`.** Three P3.3-only assumptions, each measured in the tool rather than guessed, and each the same family as the two defects the tool's own comments already record: **1.** `SERVICE_IMPORT_MODULE = f"threat_report_agent.investigation.{TARGET.stem}"` (line 71) hard-codes the package, so a move into `report/revision_writer.py` would make `service.py` import the moved names from `threat_report_agent.investigation.revision_writer` - a module that does not exist, i.e. an ImportError the moment `service` is imported. It must be built from the target's path under `src/`. **2.** The pin is NOT hard-coded (that is the good news, and the first version of this note claimed otherwise - corrected by reading the code): the tool READS the target module's `INVESTIGATION_HOST_MEMBERS` constant (`ast.literal_eval`, asserted non-empty for the hosted case, with a hostless branch). So the mechanism generalises by itself, but the CONSTANT NAME is investigation-specific and must be parametrised (`--port-constant`, defaulting to the existing name) so a `report/` module can declare `REVISION_WRITER_HOST_MEMBERS`. **3.** `MOVED_MODULE_FUNCS`/`MOVED_MODULE_CONSTS` (the P3.3b coordinator's two functions and two constants - the tool's own comment records that this hard-coded tuple was ALREADY wrong once, for P3.3d) and `SERVICE_ANCHOR` (the P3.3 layer item's `task.limitations` import) must default to empty for this slice. Verify by a dry run (no `--apply`) whose printed plan names `report/revision_writer.py` and the 2 sub-slice pin members, before any file is written.
2. **P3.4-1** - move the four pure helpers (`_select_report_evidence_rows`, `_migrate_snapshot_payload`, `_canonical_sha256`, `_snapshot_report_context`) plus `_REPORT_PROJECTION_EVIDENCE_LIMIT` into `report/revision_writer.py` with the 11-member host pin and a TRACKED contract test; their pin is only the two `cls`-reached members (`_canonical_json_chunks`, `SNAPSHOT_SCHEMA_VERSION`).
3. **P3.4-2** - move the revision lifecycle (`_create_report_revision`, `edit_report`, `get_report_revision`, `recompose_report`, `publish_report`, `submit_analyst_draft`, `workbench_submit_analyst_draft`) and `ReportComposeGateRejected` (re-exported `X as X` from `service.py` for exception identity), then record the `report.revision_writer -> models` DECISION in `recorded_allowed_edges` + `docs/plan-conflict-resolutions-20260922.md`.
4. **HARD GATES FOR EVERY SLICE - a slice missing any of these is NOT complete** (plan §4 plus this design): 1. `py scripts/check-slice-tooling.py` (+ `--self-check`); 2. the tool's own can-fail proof against a damaged subject, restored byte-for-byte; 3. `python -m compileall`; 4. the scope gate (only `allowed_files` changed); 5. `py scripts/check-import-graph.py --strict --policy docs/import-policy.json`; 6. `docker compose config`/`ps`, `--strict --services`, BLOCKED classification, never skipping `ghidra-worker`; 7. focused tests -> full suite -> `scripts/structure_behavior_probe.py --check`; 8. failure-set comparison by NODE SET, never by count; 9. deployment gate after a rebuild, then `HEAD == origin/main`; 10. the TRACKED contract test asserts the pin in both directions; 11. a delegation-shape gate (every delegation forwards every signature parameter - this phase has been bitten twice).
