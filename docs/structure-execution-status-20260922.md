...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 39 条，本文件只汇总。

- 本文档描述的树经核验于 HEAD `e50f8ab066c0907cfec40cdfc5b6094115d229e7`（本轮改动在该提交之上，与本文件一并提交）
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

## 一、为什么不是 READY / ACCEPTED

Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phase 0 and Phase 1 are complete. Phase 2 has EIGHT packages in place with 30 moved paths, report/ is COMPLETE, the ONE recorded duplicate implementation is resolved, six investigation siblings have moved, BOTH recorded import cycles are retired with an empty allowlist, and one of the two recorded legacy-path imports is gone. Phase 2 is still NOT complete: `semantic_predicates.py` remains - and it is the hardest of the seven, because `facts/dataflow.py` and `facts/thread_start.py` import it, so moving it creates upward edges - and so does P2-TK (`task/`). Phases 3-5 have not started. The deployed images DO equal the tree - 114 files x 8 services, missing=0 differing=0 container-only=0, with 42 enumerated smoke imports per container - which satisfies P5.1's mechanical condition, but the plan's own wording makes that necessary and not sufficient.
...
| phase_2 | IN PROGRESS - 8 packages in place; report/ COMPLETE; duplicates NONE recorded and NONE measured; 6 of the 7 investigation siblings moved and `semantic_predicates.py` remains (the hardest, since facts/ imports it); P2-TK task still to do |
...
114 files x 8 services, missing=0 differing=0 container-only=0, and the import smoke imports 42 enumerated modules in every container (41 before P2-V.7), with all 11 containers running
...
2. next: `semantic_predicates.py` - the LAST sibling and the only one that needs a pre-step decision, because `facts/dataflow.py` and `facts/thread_start.py` import it: moving it into `investigation/` would create upward `facts -> investigation` edges, which the policy's layer rules exist to prevent. Measure first with `.scratch/p2v2-sim-sibling.py semantic_predicates`; if the edges are forbidden, the honest options are to move the predicates DOWN into `facts/` first (the P2-V.0 shape) or to leave the module at the root with the reason recorded.
3. then P2-TK (`task/`)
