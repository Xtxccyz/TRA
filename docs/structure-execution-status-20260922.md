...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 33 条，本文件只汇总。

- 本文档描述的树经核验于 HEAD `dae1f8900a7ba484fffb0df2b7d46bd2cb0af459`（本轮改动在该提交之上，与本文件一并提交）
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

## 一、为什么不是 READY / ACCEPTED

Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phase 0 and Phase 1 are complete. Phase 2 has EIGHT packages in place with 24 moved paths, report/ is COMPLETE (including plan 7.3 step 5), the ONE recorded duplicate implementation is resolved, and BOTH recorded import cycles are retired with an empty allowlist. Phase 2 is still NOT complete: the investigation SIBLINGS themselves - `investigation_protocol.py` plus 6 more - have not moved into `investigation/`, and neither has P2-TK (`task/`). Phases 3-5 have not started. The deployed images DO equal the tree - 108 files x 8 services, missing=0 differing=0 container-only=0, with 36 enumerated smoke imports per container - which satisfies P5.1's mechanical condition, but the plan's own wording makes that necessary and not sufficient.
...
| phase_2 | IN PROGRESS - 8 packages in place; report/ COMPLETE; duplicate implementations NONE recorded and NONE measured; remaining: the 7 investigation SIBLINGS moving into investigation/ (starting with investigation_protocol.py, which P2-V measured as needing its own edge analysis) and P2-TK task |
...
108 files x 8 services, missing=0 differing=0 container-only=0, and the import smoke imports 36 enumerated modules in every container (36 before and after: a consolidation adds no module), with all 11 containers running
...
1. BOTH plan conflicts are RESOLVED and recorded (`docs/plan-conflict-resolutions-20260922.md`), BOTH import cycles are retired with `known_cycles` EMPTY, the duplicate list is EMPTY, and P2-R is COMPLETE. Nothing is blocked on the user.
2. next: move the investigation SIBLINGS into investigation/ - `investigation_protocol.py` first, then `investigation_ledger.py`, `behavior_catalog.py`, `persist_how.py`, `mechanism_completeness.py`, `mechanism_ready.py`, `semantic_predicates.py`. P2-V measured 126 collection errors when `investigation_protocol.py` moved together with the implementation, so each needs the copy-simulation edge analysis that recipe requires before it moves.
3. then P2-TK (`task/`)
