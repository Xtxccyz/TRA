...
1. **CLOSE THE OPEN VIOLATION WITH A FIXTURE THAT REACHES THE PLAN BRANCH** (the cause is now measured, so this is small): `test_speakeasy_reachability.py::test_granted_window_planning_asks_for_speakeasy` cannot observe the plan call because `service.py:10088` runs only `if sample` and the fixture supplies none. Supply a truthy `sample`, then assert the recorded plan decision is True (a monkeypatch recorder on `service.controlled_emulation_windows`) ALONGSIDE the existing request assertion, and can-fail by forcing the plan's `allow_speakeasy` argument to False - **scoped by line, because that text appears twice in `service.py`**. Two earlier attempts asserted values this fixture does not produce and were reverted by bytes; do not repeat them.
