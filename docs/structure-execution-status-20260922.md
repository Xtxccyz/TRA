...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 75 条，本文件只汇总。
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE; PHASE 3 IS IN PROGRESS and **P3.3 IS COMPLETE**. NOW COMPLETE: **P3.4** - P3.4-1 moved the assembly/selection layer behind a 2-member pin and P3.4-2 moved the revision lifecycle plus the compose-gate exception behind the design's 11-member measured pin, so `report/revision_writer.py` now owns the whole slice (`service.py` 29,640 at Phase-3 start -> 19,788). NEXT: P3.5 (`EmulationCoordinator`), then P3.6 (`WorkbenchQueryReader`) and P3.7 (the test surface), then Phases 4 and 5. P3.3b(2) and P3.3d(2) remain layer-blocked.
...
| phase_3 | IN PROGRESS - P3.1/P3.2/P3.3 COMPLETE. **P3.4 COMPLETE**: P3.4-1 (assembly/selection layer, 2-member pin) + P3.4-2 (revision lifecycle + compose-gate exception, 11-member pin) = `report/revision_writer.py` (964 lines) with `service.py` at 19,788 and all 36 call sites working through delegations. NEXT: P3.5 (`EmulationCoordinator`), P3.6 (`WorkbenchQueryReader`), P3.7 (test surface); P3.3b(2)/P3.3d(2) remain layer-blocked. |
...
1. **P3.5 (`EmulationCoordinator`)** - the plan's next slice: move the grant window, worker dispatch, result archiving and failure classification into `emulation/coordinator.py`, with the hard rule that the coordinator must NOT execute a host sample, and the success criterion that a real simulation result is separated from a placeholder record and `STATIC_BOUNDARY` is only used after a real `CONTROLLED_EMULATE`.
2. **BEFORE any further interface generation: fix the Protocol generator's `classmethod` fidelity** (recorded follow-up from the P3.4-2 review): `extend-derivation-host.py` emits `@staticmethod` but has no `classmethod` branch, so `_postgres_safe_value` and `_canonical_json_chunks` are documented as plain methods while the host defines them as classmethods, and the contract test compares names only.
3. **HARD GATES FOR EVERY SLICE** (plan section 4 plus this phase's accumulated list): 1. `py scripts/check-slice-tooling.py`; 2. a can-fail proof whose tampers are asserted to land and which restores byte-for-byte, NEVER run concurrently with the suite (it tampers src by design); 3. `python -m compileall`; 4. the scope gate; 5. `py scripts/check-import-graph.py --strict --policy docs/import-policy.json`; 6. `docker compose config`/`ps`, `--strict --services`, BLOCKED classification, never skipping `ghidra-worker`; 7. focused tests -> full suite -> `scripts/structure_behavior_probe.py --check`; 8. failure-set comparison by NODE SET; 9. the deployment gate after rebuilding BOTH image routes, then `HEAD == origin/main`; 10. the tracked contract test asserts the pin in both directions; 11. the delegation-shape gate (parameters, and `__doc__` equality).
