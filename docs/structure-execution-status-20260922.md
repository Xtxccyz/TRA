...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 50 条，本文件只汇总。

- 本文档描述的树经核验于 HEAD `edc8609480aba50d879c990b16f6972ddc97a3ea`（本轮改动在该提交之上，与本文件一并提交）
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

## 一、为什么不是 READY / ACCEPTED

Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phase 0, Phase 1 and Phase 2 are COMPLETE (nine packages, 34 moved paths, report/ finished, all seven investigation siblings moved, all three task modules moved, duplicate list empty, cycle allowlist empty). PHASE 3 IS IN PROGRESS: P3.1 (facade contract stated and pinned), P3.2-design (the `TaskHost` port declared with its contract test), P3.2a/P3.2b (the failure/limitation projection seam extracted) P3.2c (the `creation` cluster moved behind the port - `create_submission_task`, `prepare_blind_run` and the `SubmissionResult` dataclass) P3.2d (the `lifecycle` cluster - `archive_case`) and P3.2e (the `budget` cluster - `_deferred_budget_thread_ids` and `_actual_depth`, the latter a static method that needed no host at all) are COMPLETE, leaving one-statement delegations in service.py. NOT STARTED: P3.2f (cancellation, ~206 lines, the only cluster needing `_seal_task_audit_chain`) and P3.2g (workbench binding, ~94 lines, the ONE cluster that widens the port), P3.3 (InvestigationCoordinator), P3.4 (ReportRevisionWriter), P3.5 (EmulationCoordinator), P3.6 (WorkbenchQueryReader), P3.7 (test surface off private/`getsource`), and Phases 4 and 5. The deployed images DO equal the tree - 121 files x 8 services, missing=0 differing=0 container-only=0, with 49 enumerated smoke imports per container - which satisfies P5.1's mechanical condition, but the plan's own wording makes that necessary and not sufficient. See `docs/p32-task-runner-design-20260922.md` for the measured P3.2 scale, the port and the per-cluster order.
...
| phase_3 | IN PROGRESS - P3.1, P3.2-design, P3.2a, P3.2b, P3.2c, P3.2d and P3.2e COMPLETE (service.py 29,640 lines at Phase-3 start -> 29,175). The blocker this entry used to record is RESOLVED: the port exists, is re-derived from source by its contract test every run, and has three clusters behind it. Remaining: P3.2f (cancellation), P3.2g (workbench binding, the port-widening one), then P3.3-P3.7. |
...
2. Continue with P3.2f (cancellation: `cancel_task` + `cancel_tool_run`, ~206 lines, the only cluster needing `_seal_task_audit_chain`): run `py .scratch/p32-measure-cluster.py cancel_task cancel_tool_run` FIRST, then `p32-extract-cluster.py`, `p32-verify-cluster.py`, `p32-canfail-cluster.py`, the battery, a rebuild and the deployment gate. P3.2g (workbench binding) is the one that widens the port by `_context_payload_v3`, `_context_state_for_task_v3` and `_require_session_id`, and its two entry points are a 2-line forwarder plus a 90-line implementation.
