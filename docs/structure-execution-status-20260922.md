...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 78 条，本文件只汇总。

- **被核验的树 = 提交 `1cadc395afb45c61eadcceefcf9b7fbaaf4de07d`**（该提交的 tree 上跑过四道门禁与全量套件）
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE, P3.1-P3.4 are COMPLETE and **P3.6-1 IS COMPLETE** (the five read-only workbench views plus two view helpers in `workbench_query.py`, 11-member pin, read-only gate; `service.py` 29,640 at Phase-3 start -> 18,831). **P3.5 IS BLOCKED** (measured layer conflict, plan conflict 3) and P3.6 still owes `workbench_capabilities` (needs `ActionCatalog`). ONE preparation step ('P3.5/P3.6-0': sink the `investigation/` contracts into `contracts.py`, route tool execution through `ports.ToolExecutionPort` with the executor injected, record the `models` and `emulation.policy` decisions) unblocks both. The owed P3.6-1 two-axis review is PAID (decision (f) registered). Then the shared preparation step, P3.6-2/P3.5-2, P3.7 and Phases 4-5; P3.3b(2)/P3.3d(2) remain layer-blocked.
...
| phase_3 | IN PROGRESS - P3.1-P3.4 COMPLETE; **P3.6-1 COMPLETE** (the read-only workbench views in `workbench_query.py`, 11-member pin, read-only gate; service.py 18,831). **P3.5 BLOCKED** (plan conflict 3). Owed: the P3.6-1 two-axis review; then the shared preparation step P3.5/P3.6-0; then P3.6-2/P3.5-2, P3.7, Phases 4-5. P3.3b(2)/P3.3d(2) remain layer-blocked. |
...
PASS - 130 files per service across 8 services, missing=0 differing=0 container-only=0, import smoke 57 enumerated modules per service, 11 containers running
...
1. **THEN P3.5/P3.6-0 (the shared preparation step)**: sink the thirteen `investigation/` contracts plus `ActionCatalog` into `contracts.py` with `X as X` re-exports; route tool execution through `ports.ToolExecutionPort` (executor injected, becomes a host-pin member); sink `TaskLifecycle`/`PackageEntry`; record the `models` and `emulation.policy` decisions. NOTE THE SCOPE: it touches `contracts.py`/`ports.py`/`investigation/**`, which are outside P3.5's and P3.6's allowed files - write the scope note in the step record.
2. **THEN P3.6-2** (`workbench_capabilities`, once `ActionCatalog` is reachable) and **P3.5-2** (17 methods + 2 constants, 6-member pin) once conflict 3 is resolved.
3. **HARD GATES FOR EVERY SLICE**: 1. `py scripts/check-slice-tooling.py`; 2. `py .scratch/canfail.py --plan <slice plan>.json` (data-driven; never concurrently with the suite); 3. `python -m compileall`; 4. the scope gate; 5. `py scripts/check-import-graph.py --strict --policy docs/import-policy.json`; 6. `docker compose config`/`ps`, `--strict --services`, BLOCKED classification, never skipping `ghidra-worker`; 7. focused tests -> full suite -> `scripts/structure_behavior_probe.py --check`; 8. failure-set comparison by NODE SET; 9. the deployment gate after rebuilding BOTH image routes, then `HEAD == origin/main`; 10. the tracked contract test asserts the pin in both directions; 11. the delegation-shape gate (parameters AND `__doc__` equality); 12. for read-only slices, the read-only AST assertion.
4. **A READER ANALYSIS MUST INCLUDE EVERY HOST PIN IN THE REPOSITORY**, not just `self.`/`cls.` readers inside `service.py`: the P3.6-1 defect (37 broken tests) came from a class constant read through another module's host pin. Before moving any class attribute, grep the `*_HOST_MEMBERS` tuples of every migrated module for its name.
5. **P3.6-2 debt recorded by the review, to close when `workbench_capabilities` moves**: (a) the design's section 5.4 dict-key-set assertion is still missing - add a direct comparison of the view's returned key set before/after; (b) the read-only assertion's receiver list matches the design's prescribed list and therefore misses `session.execute(update(...))` and derived handles - widen it when the slice is touched next; (c) `_UNIQUE_THREAD_VIEW_KINDS` is pinned conservatively although the design's rule would let it travel - re-measure with the repository-wide host-pin scan before moving it.
