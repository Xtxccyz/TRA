...
2. The next P3.3 step is P3.3e: move `_derive_investigation_observations` WHOLESALE, together with the four exclusive helpers (`_follow_local_tail_jmp`, `_global_accesses_from_row`/`_global_accesses_from_rows`, `_row_own_function_matches`, `_matching_simulation_results`) and the three module-level functions (`_decode_output_buffer`, `_bind_recovered_xor_verification`, `plausible_traced_creation_flags`). The port must grow by the six shared helpers listed in `docs/p33ef-giants-decision-20260922.md` section 1.1, and the new module must import `facts.dataflow` (NOT the legacy `threat_report_agent.dataflow`).
