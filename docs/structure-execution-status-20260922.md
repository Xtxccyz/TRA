...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 71 条，本文件只汇总。

- **被核验的树 = 提交 `169bca2cbaff922f16ca084bb5ef916cb0e886bd`**（该提交的 tree 上跑过四道门禁与全量套件）
- `head_sha` 的语义：**`head_sha` 是被门禁核验的代码提交 `169bca2cbaff`，不是「当前 HEAD」。** 四道门禁、focused 套件与全量套件都在它的 tree 上运行过。**本轮实测到的两处漂移正是这个字段造成的**：先前它记的是提交前的 HEAD，于是文档声称在一个不含本步改动的提交上完成核验；改成「当前 HEAD」后又发现，写下该值的提交本身就会移动 HEAD——**任何文件都无法正确写出「包含自己的那个提交」**。因此这里固定记代码提交，并在每次复验时核对 `src/` 与 `tests/` 是否仍与它一致（本轮实测：`git diff --name-only 169bca2cbaff..HEAD -- src tests` 为空，即逐字节相同）。每一步的回滚点是该步 `rollback_point` 记录的上一个提交。 ROUND 116 UPDATE: the verified code commit is now `169bca2cbaff`; the measured check above was re-run for it - `git diff --name-only 169bca2cbaff..HEAD -- src tests` is empty (HEAD == 169bca2cbaff at the moment the records were written), and the deployment gate's own output names that commit: `checked 126 file(s) across 8 service(s) against HEAD 169bca2cbaff`. UPDATE for 169bca2cbaff: the verified code commit is now `169bca2cbaff`; the measured check above was re-run - `git diff --name-only 169bca2cbaff..HEAD -- src tests` is empty (HEAD == 169bca2cbaff when the records were written), and the deployment gate's own output names that commit. UPDATE for 169bca2cbaff: the verified code commit is now `169bca2cbaff`; the measured check above was re-run - `git diff --name-only 169bca2cbaff..HEAD -- src tests` is empty (HEAD == 169bca2cbaff when the records were written), and the deployment gate's own output names that commit. UPDATE: the verified CODE commit is STILL `169bca2cbaff` - the commits after it are documentation and records only, and that is proven rather than asserted: `git diff --name-only 169bca2cbaff..1f82e634b4ed -- src tests` is EMPTY, so the gates and the suites that ran on 169bca2cbaff apply to this tree unchanged. HEAD is 1f82e634b4ed.
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

### 读取指引：本文件的哪一部分代表**当前**状态

- **权威内容＝`step_records` + 最新的 `final_state_*` 对象（当前是 `final_state_round_120_p33f1`，由 `authoritative_final_state` 指定）。**
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE; PHASE 3 IS IN PROGRESS. P3.1/P3.2 COMPLETE; P3.3 has four slices (b/a/c/d), P3.3c(2), three layer items, **P3.3e COMPLETE** (the 2,787-line giant in `investigation/derivation.py`), the P3.3f DESIGN and now **P3.3f-1 COMPLETE**: the two clusters that blocked the loop's move are sunk, so the loop's 63 free names contain no forbidden-layer name at all. `service.py` is at 24,450 lines. NEXT: **P3.3f-2**, the move itself (3,523 lines, a 21-component port measured before the sink and to be re-measured at move time, 13 module-level travellers of which `_investigation_scheduled_keys` is the only one left to carry, two `getsource` sites and one monkeypatch site to migrate). Then P3.4-P3.7. P3.3b(2) and P3.3d(2) remain layer-blocked.
...
| phase_3 | IN PROGRESS - P3.1/P3.2 COMPLETE. P3.3: four slices (P3.3b/a/c/d) + P3.3c(2) + three layer items + P3.3e COMPLETE + P3.3f DESIGNED + **P3.3f-1 COMPLETE** (the autopsy cluster and the seed/evidence-key cluster sunk into `investigation/`, which removed the loop's ONLY forbidden-layer name). service.py 29,640 -> 24,450. NEXT: P3.3f-2, the 3,523-line loop's move, then P3.4-P3.7; P3.3b(2)/P3.3d(2) remain layer-blocked. |
...
1. **P3.3f-2 - THE MOVE.** `_run_investigation_loop` (3,523 lines) plus its port, its 7 nested closures (1 carrying receiver references) and `_investigation_scheduled_keys`. RE-MEASURE the port at move time with `.scratch/p33f-member-homes.py` and `.scratch/p33e-cluster-fixed-point.py --member _run_investigation_loop`: this step has changed BOTH inputs (the twelve seed names and the autopsy name now live in allowed layers, so they cost no port entry at all, while `_investigation_scheduled_keys` and whatever else the loop reads through the receiver still do). Prefer landing the loop in `investigation/derivation.py`, where six already-moved calls become BARE. Migrate the two `getsource` sites and re-check the instance-level monkeypatch site, and expect more than a grep predicts - P3.3e predicted none and found three, this slice found three before moving.
2. **THE REPOSITORY IS IN SYNC** as of this step's commits. Push route, measured repeatedly: retry BOTH `git -c http.proxy= push https://github.com:443/Xtxccyz/TRA.git HEAD:refs/heads/main` and the same with `-c http.proxy=http://127.0.0.1:7897`; the explicit `:443` avoids the broken `ghproxy.net` rewrite.
