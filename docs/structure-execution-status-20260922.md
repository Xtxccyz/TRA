...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 59 条，本文件只汇总。

- **被核验的树 = 提交 `a48e902c2966159a28822c30d8f41d3b43f3d785`**（该提交的 tree 上跑过四道门禁与全量套件）
- `head_sha` 的语义：**`head_sha` 是被门禁核验的代码提交 `a48e902c2966`，不是「当前 HEAD」。** 四道门禁、focused 套件与全量套件都在它的 tree 上运行过。**本轮实测到的两处漂移正是这个字段造成的**：先前它记的是提交前的 HEAD，于是文档声称在一个不含本步改动的提交上完成核验；改成「当前 HEAD」后又发现，写下该值的提交本身就会移动 HEAD——**任何文件都无法正确写出「包含自己的那个提交」**。因此这里固定记代码提交，并在每次复验时核对 `src/` 与 `tests/` 是否仍与它一致（本轮实测：`git diff --name-only a48e902c2966..HEAD -- src tests` 为空，即逐字节相同）。每一步的回滚点是该步 `rollback_point` 记录的上一个提交。
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

### 读取指引：本文件的哪一部分代表**当前**状态

- **权威内容＝`step_records` + 最新的 `final_state_*` 对象（当前是 `final_state_round_104_layer1`，由 `authoritative_final_state` 指定）。**
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE (nine packages, 34 moved paths, all seven investigation siblings and all three task modules moved, duplicate list empty, cycle allowlist empty). PHASE 3 IS IN PROGRESS. P3.1 (facade contract) and the whole of P3.2 are COMPLETE (the `TaskHost` port has 9 members, all used, with eleven functions behind it). P3.3 is DESIGNED (53 candidates / 8,881 lines / 20-member spine, six slices, 495 lines excluded to P3.4/P3.6 by name) and FIVE PIECES OF IT HAVE MOVED: four slices into `investigation/coordinator.py`, whose port is now SIX members (`database`, `_audit`, `_MAX_COMPLETED_ACTION_EVIDENCE_IDS`, `_CONVERGENCE_ALTERNATES`, `_CONVERGENCE_EXPECTED_KINDS`, `_canonical_json`) - P3.3b (frontier helpers), P3.3a (ledger), P3.3c (action proposal) and P3.3d (convergence contract) - plus LAYER ITEM 1, the new module `investigation/loop_path.py` (16 names / 167 lines) holding the loop-path and persist-HOW SKIP policy, which REMOVED the `investigation -> task` import cycle. `service.py` is down from 29,640 lines at the Phase-3 start to 28,028. STILL BLOCKED ON LAYERS RATHER THAN EFFORT: P3.3b(2) (a helper must move below `report/`), P3.3c(2) (the model port must expose `DynamicPlanAction`; its OTHER blocker, `action_is_model_or_human` being unreachable from `investigation/`, is now GONE) and P3.3d(2) (`methodology` must become a declared shared module or its fact library must move down). P3.3e (2,795 lines; 3,041 wholesale) is blocked on exposing 7 `simulation_adapters` policy names as an allowed emulation interface; P3.3f (3,523 lines) lost its cycle blocker but still needs ~34 shared helpers sunk and the `getsource` assertion migrated (`tests/test_analysis_task_orchestration.py:404`). NOT STARTED: P3.4 (ReportRevisionWriter), P3.5 (EmulationCoordinator), P3.6 (WorkbenchQueryReader), P3.7 (test surface off private/`getsource`), and Phases 4 and 5. Per-slice detail, including every measured blocker: `docs/p33-investigation-coordinator-design-20260922.md`, `docs/p33ef-giants-decision-20260922.md` and `docs/p33-layer1-loop-path-module-design-20260922.md`.
...
| phase_3 | IN PROGRESS - P3.1 and P3.2 COMPLETE (TaskHost; service.py 29,640 -> 28,898). P3.3 DESIGNED with FOUR SLICES MOVED (P3.3b/a/c/d; coordinator port = six members; service.py 28,898 -> 28,028) AND THE FIRST LAYER ITEM DONE (`investigation/loop_path.py`; the `investigation -> task` cycle is gone). The two giant methods are DECIDED, not pending: P3.3e moves wholesale (3,041 lines = 2,795 + 165 + 74 + 7, port 6 -> 12) once the simulation-adapter interface exists; P3.3f (3,523 lines) is no longer cycle-blocked but still needs ~34 shared helpers sunk and one `getsource` assertion migrated. Three further sub-slices (P3.3b(2), P3.3c(2), P3.3d(2)) stay blocked on layers. |
...
2. P3.3b/a/c/d AND LAYER ITEM 1 are DONE - do not re-open them. Do not start P3.3e or P3.3f either: their remaining blockers are LAYERS, not effort. The next executable structural step is LAYER ITEM 4 in `docs/p33ef-giants-decision-20260922.md` section 3 (make the model port expose `DynamicPlanAction`), because it is the smallest and purest of the five and it is the LAST blocker on P3.3c(2)'s three members / 71 lines. Do LAYER ITEM 2 next (expose the 7 `simulation_adapters` policy names as an allowed emulation interface): it is the only thing that unblocks P3.3e, the largest remaining slice. Items 3 and 5 are recorded there with their measurements.
3. MEASURE BEFORE MOVING, and do not trust the previous step's tooling blindly: the P3.3 layer-item-1 move shipped a draft whose span started at `node.lineno`, which DROPPED `@dataclass(frozen=True)` from the moved class and RE-BOUND it onto `class AnalysisTaskRuntime(Protocol)` in the source module. Every gate stayed green (import graph, structure diff, behaviour probe, `compileall`, and a byte-identity verifier that shared the extractor's blind spot); only the pytest battery caught it. Any ATOMIC SPAN MUST START AT THE FIRST DECORATOR (`min(node.lineno, *(d.lineno for d in node.decorator_list))`), and any move must DERIVE the imports its new home needs from the moved body's free names instead of listing them by hand - the hand-written list in that step was missing BOTH `dataclass` and `SimpleNamespace`.
4. WHEN A TOOL AND ITS VERIFIER ARE WRITTEN TOGETHER, TEST THE VERIFIER AGAINST A DELIBERATELY DAMAGED SUBJECT, or it will agree with the tool by construction. This is the sixth narrow-scan defect of the phase (`cls.` receivers, working-tree reads while measuring, shape-without-arity, a vacuous can-fail proof, `AnnAssign`, now `decorator_list`) and the second time a verifier inherited the blind spot it existed to detect.
5. `.scratch/restore-from-head.py --rev <rev> [--delete <untracked>] <paths...>` restores tracked files byte-for-byte with hash verification and deletes untracked leftovers; it never uses `git reset --hard`, `checkout --` or `clean`. Use it to undo a failed attempt from a known-good base instead of hand-editing.
6. `structure_status=READY` must never imply `capability_status=ACCEPTED`: the Ghidra B3/C3 capability items are untouched by this plan.
