...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 38 条，本文件只汇总。

- 本文档描述的树经核验于 HEAD `0b29aa7d324172b167f9a52c56569cee653e0d9a`（本轮改动在该提交之上，与本文件一并提交）
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

## 一、为什么不是 READY / ACCEPTED

Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phase 0 and Phase 1 are complete. Phase 2 has EIGHT packages in place with 29 moved paths, report/ is COMPLETE, the ONE recorded duplicate implementation is resolved, five investigation siblings have moved, and BOTH recorded import cycles are retired with an empty allowlist. Phase 2 is still NOT complete: 2 investigation siblings remain (`persist_how.py`, `semantic_predicates.py`) and so does P2-TK (`task/`). Phases 3-5 have not started. The deployed images DO equal the tree - 113 files x 8 services, missing=0 differing=0 container-only=0, with 41 enumerated smoke imports per container - which satisfies P5.1's mechanical condition, but the plan's own wording makes that necessary and not sufficient.
...
| phase_2 | IN PROGRESS - 8 packages in place; report/ COMPLETE; duplicates NONE recorded and NONE measured; 5 of the 7 investigation siblings moved and 2 remain (`persist_how`, `semantic_predicates`); P2-TK task still to do |
...
113 files x 8 services, missing=0 differing=0 container-only=0, and the import smoke imports 41 enumerated modules in every container (40 before P2-V.6), with all 11 containers running
...
2. next: `persist_how.py` - the only sibling with package imports (7, plus 1 function-level one) and the subject of the recorded `persist_how -> reporting` violation, so it is the most entangled of the seven. Then `semantic_predicates.py` LAST: `facts/dataflow.py` and `facts/thread_start.py` import it, so moving it creates upward `facts -> investigation` edges that need their own edge analysis and possibly a pre-step (like P2-V.0 was for `investigation.py`).
3. then P2-TK (`task/`)
