...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 35 条，本文件只汇总。

- 本文档描述的树经核验于 HEAD `5401101b8b5473ab6da76c139d3411e55674c402`（本轮改动在该提交之上，与本文件一并提交）
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

## 一、为什么不是 READY / ACCEPTED

Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phase 0 and Phase 1 are complete. Phase 2 has EIGHT packages in place with 26 moved paths, report/ is COMPLETE, the ONE recorded duplicate implementation is resolved, two investigation siblings have moved, and BOTH recorded import cycles are retired with an empty allowlist. Phase 2 is still NOT complete: 5 investigation siblings remain (`behavior_catalog.py`, `persist_how.py`, `mechanism_completeness.py`, `mechanism_ready.py`, `semantic_predicates.py`) and so does P2-TK (`task/`). Phases 3-5 have not started. The deployed images DO equal the tree - 110 files x 8 services, missing=0 differing=0 container-only=0, with 38 enumerated smoke imports per container - which satisfies P5.1's mechanical condition, but the plan's own wording makes that necessary and not sufficient.
...
| phase_2 | IN PROGRESS - 8 packages in place; report/ COMPLETE; duplicates NONE recorded and NONE measured; 2 of the 7 investigation siblings moved (`investigation_protocol`, `investigation_ledger`) and 5 remain; P2-TK task still to do. The sibling simulation is reusable: `.scratch/p2v2-sim-sibling.py <module>` |
...
110 files x 8 services, missing=0 differing=0 container-only=0, and the import smoke imports 38 enumerated modules in every container (37 before P2-V.3), with all 11 containers running
...
2. next: the remaining 5 investigation siblings, one per step, each through `.scratch/p2v2-sim-sibling.py <module>` plus `.scratch/p2v2-sim-sanity.py`. Suggested order by measured hazard: `behavior_catalog.py` (imported by the implementation AND by report/analyst_report), `mechanism_completeness.py`, `mechanism_ready.py`, `persist_how.py` (the only sibling with package imports, and the subject of the recorded `persist_how -> reporting` violation), and `semantic_predicates.py` LAST because `facts/dataflow.py` and `facts/thread_start.py` import it, so moving it creates upward edges.
3. then P2-TK (`task/`)
