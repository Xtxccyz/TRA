...
| phase_3 | IN PROGRESS - P3.1, P3.2-design, P3.2a, P3.2b and P3.2c-P3.2f COMPLETE (service.py 29,640 lines at Phase-3 start -> 28,984; the whole 6-member TaskHost port is now load-bearing and fully used). Remaining in P3.2: P3.2g (workbench binding, the port-widening one). Then P3.3 (InvestigationCoordinator), P3.4 (ReportRevisionWriter), P3.5 (EmulationCoordinator), P3.6 (WorkbenchQueryReader) and P3.7. |
...
2. Continue with P3.2g (workbench binding: `bind_historical_analysis` is a 2-line forwarder to `workbench_bind_existing_analysis`, whose 90 lines need `_context_payload_v3`, `_context_state_for_task_v3` and `_require_session_id`). This is the ONE cluster that deliberately WIDENS `TASK_HOST_MEMBERS`: widen the pin and the `TaskHost` declaration in the same change, record why, and expect `tests/test_task_runner_contract.py::test_the_port_is_exactly_what_the_task_path_needs` to fail until the pin matches. Then P3.3-P3.7.
