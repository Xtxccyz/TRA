...
1. **PUSH PENDING - THE REPOSITORY IS NOT IN SYNC.** Local HEAD is `4846f59c8e32` and `origin/main` is `8059cfc2f92b`; 2 verified commit(s) are local only. The cause is NETWORK, not credentials or policy: TCP to github.com:22/443 and ssh.github.com:443 connects, but the SSH handshake is reset ('Connection reset by 20.205.243.160 port 443') or times out, over both routes, with no HTTP(S)/ALL proxy in the environment; retried four times. RETRY THE PUSH FIRST, and do not describe the repository as in sync until `git rev-parse HEAD` equals `git rev-parse origin/main`.
