...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 74 条，本文件只汇总。

- **被核验的树 = 提交 `e3ef17a6a5b49d1a68f9643087616ba5be828e1d`**（该提交的 tree 上跑过四道门禁与全量套件）
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE; PHASE 3 IS IN PROGRESS and **P3.3 IS COMPLETE**. NOW COMPLETE: **P3.4-1**, the revision-writer assembly layer (4 members + the Evidence window constant) in `report/revision_writer.py` behind a 2-member measured host pin, with a tracked contract test, a three-tamper can-fail proof, byte-identical bodies and a clean deployment gate (`service.py` 20,486 -> 20,159 lines). NEXT: P3.4-2 (the revision lifecycle: `_create_report_revision`, `edit_report`, `get_report_revision`, `recompose_report`, `publish_report`, `submit_analyst_draft`, `workbench_submit_analyst_draft`, `ReportComposeGateRejected`), then P3.5-P3.7, then Phases 4 and 5. P3.3b(2) and P3.3d(2) remain layer-blocked.
...
| phase_3 | IN PROGRESS - P3.1/P3.2/P3.3 COMPLETE. P3.4: **P3.4-1 COMPLETE** (the revision-writer assembly layer in `report/revision_writer.py`, 2-member measured pin, tracked contract test, 3-tamper can-fail proof, byte-identical bodies, deployment PASS 129x8; service.py 20,486 -> 20,159). NEXT: P3.4-2 (the revision lifecycle + the compose-gate exception), then P3.5-P3.7; P3.3b(2)/P3.3d(2) remain layer-blocked. |
...
1. **P3.4-2 (the next step)** - move the revision lifecycle into the SAME module and extend the pin from 2 to the design's 11 members: `_create_report_revision` (156), `edit_report` (52), `get_report_revision` (28), `recompose_report` (44), `publish_report` (25), `submit_analyst_draft` (96), `workbench_submit_analyst_draft` (32), plus `ReportComposeGateRejected` defined in the new module and re-exported `X as X` from `service.py` (identity assertion required). The 11-member pin is in the design's section 2 with a reason per member; `_report/writer`'s pin constant is `REVISION_WRITER_HOST_MEMBERS` and the mover now takes `--port-constant`.
2. **P3.4-2 MUST keep the draft-gate behaviour identical**: `test_workbench_report_file.py`'s three guards (a passing draft has no `gate_violations`; a rejected draft leaves the official revision untouched; the route answers 422) must pass unchanged, and `edit_report`'s gateless `status="DRAFT"` write must remain gateless - it is the reason a draft cannot become official by itself.
3. **HARD GATES FOR EVERY SLICE** (plan section 4 plus the design): 1. `py scripts/check-slice-tooling.py`; 2. a can-fail proof whose tampers are asserted to land and which restores byte-for-byte; 3. `python -m compileall`; 4. the scope gate (only `allowed_files`, plus the gate baselines a move legitimately touches); 5. `py scripts/check-import-graph.py --strict --policy docs/import-policy.json`; 6. `docker compose config`/`ps`, `--strict --services`, BLOCKED classification, never skipping `ghidra-worker`; 7. focused tests -> full suite -> `scripts/structure_behavior_probe.py --check`; 8. failure-set comparison by NODE SET; 9. the deployment gate after rebuilding BOTH image routes, then `HEAD == origin/main`; 10. the tracked contract test asserts the pin in both directions; 11. the delegation-shape gate.
