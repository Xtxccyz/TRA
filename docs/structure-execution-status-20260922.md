...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 28 条，本文件只汇总。

- 本文档描述的树经核验于 HEAD `6b3eaa9bdc0aa5c595825517ef94d48e02b06cb9`（本轮改动在该提交之上，与本文件一并提交）
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

## 一、为什么不是 READY / ACCEPTED

Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phase 0 and Phase 1 are complete. Phase 2 has SEVEN packages in place (facts, report with 3 of 4 modules, intake, static, emulation, investigation, model) and `docs/import-policy.json` records 21 moved paths, but Phase 2 is NOT complete: `reporting.py`, the 7 investigation siblings that were measured to need their own edge analysis, P2-T (`tools/`) and P2-TK (`task/`) remain. Phases 3-5 have not started. The deployed images DO equal the tree - 103 files x 8 services, missing=0 differing=0 container-only=0, with 30 enumerated smoke imports per container - which satisfies P5.1's mechanical condition, but the plan's own wording makes that necessary and not sufficient.
...
| phase_2 | IN PROGRESS - 7 packages in place (facts, report 3/4, intake, static, emulation, investigation, model); remaining: report/reporting.py and the tests-only `document_to_markdown` exit, the 7 investigation siblings, P2-T tools, P2-TK task |
...
103 files x 8 services, missing=0 differing=0 container-only=0, and the import smoke imports 30 enumerated modules in every container (25 before P2-M; the package list is now DERIVED from disk, so a package created by a later step cannot be silently left unsmoked)
...
1. BOTH plan conflicts are RESOLVED and recorded (`docs/plan-conflict-resolutions-20260922.md`): the asset directory was renamed `static/` -> `assets/` with the `/static` mount URL kept, and the module -> package moves use the shared lazy facade in `src/threat_report_agent/package_facade.py`. Nothing is blocked on the user.
...
3. then P2-T (`tool_execution.py`, `tool_authoring.py` -> `tools/`) and P2-TK (`task/`), each through the 9-step algorithm in plan 7.1, and then the 7 investigation siblings that P2-V measured as needing their own edge analysis (`investigation_protocol.py` produced 126 collection errors when moved with the implementation)
