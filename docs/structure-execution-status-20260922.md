...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 70 条，本文件只汇总。

- **被核验的树 = 提交 `d2bb1f51a1e928f6f6e17624cb750cfb66f7ea51`**（该提交的 tree 上跑过四道门禁与全量套件）
- `head_sha` 的语义：**`head_sha` 是被门禁核验的代码提交 `8a021953fd95`，不是「当前 HEAD」。** 四道门禁、focused 套件与全量套件都在它的 tree 上运行过。**本轮实测到的两处漂移正是这个字段造成的**：先前它记的是提交前的 HEAD，于是文档声称在一个不含本步改动的提交上完成核验；改成「当前 HEAD」后又发现，写下该值的提交本身就会移动 HEAD——**任何文件都无法正确写出「包含自己的那个提交」**。因此这里固定记代码提交，并在每次复验时核对 `src/` 与 `tests/` 是否仍与它一致（本轮实测：`git diff --name-only 8a021953fd95..HEAD -- src tests` 为空，即逐字节相同）。每一步的回滚点是该步 `rollback_point` 记录的上一个提交。 ROUND 116 UPDATE: the verified code commit is now `8a021953fd95`; the measured check above was re-run for it - `git diff --name-only 8a021953fd95..HEAD -- src tests` is empty (HEAD == 8a021953fd95 at the moment the records were written), and the deployment gate's own output names that commit: `checked 126 file(s) across 8 service(s) against HEAD 8a021953fd95`. UPDATE for 8a021953fd95: the verified code commit is now `8a021953fd95`; the measured check above was re-run - `git diff --name-only 8a021953fd95..HEAD -- src tests` is empty (HEAD == 8a021953fd95 when the records were written), and the deployment gate's own output names that commit. UPDATE for 8a021953fd95: the verified code commit is now `8a021953fd95`; the measured check above was re-run - `git diff --name-only 8a021953fd95..HEAD -- src tests` is empty (HEAD == 8a021953fd95 when the records were written), and the deployment gate's own output names that commit. UPDATE: the verified CODE commit is STILL `8a021953fd95` - the commits after it are documentation and records only, and that is proven rather than asserted: `git diff --name-only 8a021953fd95..1f82e634b4ed -- src tests` is EMPTY, so the gates and the suites that ran on 8a021953fd95 apply to this tree unchanged. HEAD is 1f82e634b4ed.
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

### 读取指引：本文件的哪一部分代表**当前**状态

- **权威内容＝`step_records` + 最新的 `final_state_*` 对象（当前是 `final_state_round_119_p33fdesign`，由 `authoritative_final_state` 指定）。**
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE; PHASE 3 IS IN PROGRESS and **P3.3e IS COMPLETE** (the 2,787-line derivation giant moved with a 7-member measured port). NOW DESIGNED AND MEASURED: **P3.3f, `_run_investigation_loop` (3,523 lines, the phase's largest slice)** - 45 receiver references, a 21-component port, 13 module-level travellers (12 needing a sink), one forbidden name removable in 87 lines, two `getsource` sites and one monkeypatch site. Its execution is split into P3.3f-1 (sink the autopsy cluster and the seed/evidence-key cluster) and P3.3f-2 (the move). `service.py` is at 24,948 lines. AFTER P3.3f: P3.4-P3.7, then Phases 4 and 5. P3.3b(2) and P3.3d(2) remain layer-blocked.
...
| phase_3 | IN PROGRESS - P3.1/P3.2 COMPLETE. P3.3: four slices (P3.3b/a/c/d) + P3.3c(2) + three layer items + P3.3e COMPLETE (the 2,787-line giant in `investigation/derivation.py`, 7-member measured port) + **P3.3f DESIGNED** (the 3,523-line loop: 21-component port, 13 travellers of which 12 need a sink, one forbidden name removable in 87 lines, two getsource sites). service.py 29,640 -> 24,948. NEXT: P3.3f-1 (the sink preparation) then P3.3f-2 (the move), then P3.4-P3.7; P3.3b(2)/P3.3d(2) remain layer-blocked. |
...
1. **P3.3f-1 (the preparation step)** - sink `no_new_evidence_autopsy` + `_first_selector_value` + `_has_nonempty` + `NO_NEW_EVIDENCE_CATEGORIES` + `_AUTOPSY_NEXT_ACTIONS` (87 lines + 2 constants, closure measured) into `investigation/` with a re-export from `deep_analysis_quality.py`; then sink the seed/evidence-key cluster (the 12 names listed in the design's section 4) keeping every name reachable from `service.py`, because ten test files read them.
2. **P3.3f-2 (the move)** - move the loop with its measured port, its 7 closures (1 carrying receiver references), `_investigation_scheduled_keys`, and migrate the two `getsource` sites; re-check the instance-level monkeypatch site. Prefer landing it in `investigation/derivation.py`, where six of its already-moved calls become BARE and cost no port entry - and STATE which home was chosen and why.
