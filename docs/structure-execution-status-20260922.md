...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 58 条，本文件只汇总。
...
- **权威内容＝`step_records` + 最新的 `final_state_*` 对象（当前是 `final_state_round_103_giants`，由 `authoritative_final_state` 指定）。**
...
| phase_3 | IN PROGRESS - P3.1 and P3.2 COMPLETE (TaskHost; service.py 29,640 -> 28,898). P3.3 DESIGNED with FOUR SLICES MOVED (P3.3b/a/c/d; port = six members; service.py 28,898 -> 28,028). The two giant methods are now DECIDED, not pending: P3.3e moves wholesale (3,124 lines, port 6 -> 12), P3.3f is BLOCKED on the `investigation -> task` cycle with a recorded unblocking order. Three further sub-slices stay blocked on layer moves: P3.3b(2), P3.3c(2), P3.3d(2). Then P3.4-P3.7. |
...
2. P3.3b/a/c/d are DONE; P3.3e and P3.3f are NOT movable yet and the reason is a LAYER, not effort - do not start either without doing the layer item first. The next executable step is LAYER ITEM 1 from `docs/p33ef-giants-decision-20260922.md` section 3: move `LOOP_PATH_PERSIST_READY` / `LOOP_PATH_PERSIST_BOUNDARY` / `LOOP_PATH_BUDGET_DEFER` / `next_investigation_loop_path` / `resolve_persist_how_skip` out of `task/analysis_task_orchestration.py` into a layer `investigation/` may import, re-export from task, and migrate `tests/test_analysis_task_orchestration.py:402`'s `getsource(AnalysisService._run_investigation_loop)` assertion. That single step unblocks P3.3f AND P3.3c(2).
3. Before ANY move, run the pre-move gates listed in that document section 1.4: the reference check (`.scratch/p33-reference-check.py`), the arity check (`p33-arity.py`), and the three identity tools - and remember they prove identity MODULO the receiver rewrite, not correct wiring.
4. Do NOT attempt P3.3f before the layer work: push `next_investigation_loop_path` / `resolve_persist_how_skip` / `LOOP_PATH_*` out of `task/analysis_task_orchestration.py` into a layer `investigation/` may import, which also unblocks P3.3c(2). Then sink the pure `_select_*`/`_is_*` helpers so the port does not have to grow to ~40.
5. The standing tools and their limits are recorded in the decision document: use all three (AST identity, string values + code lines, call arity) for any move, and remember that none of them proves that the moved helpers are CORRECTLY WIRED - that is what the focused battery and the full-suite failure-node comparison are for.
6. P3.3b, P3.3a, P3.3c and P3.3d are DONE - do not re-open them. Before the next slice, run `py .scratch/p32-measure-cluster.py <members> --from <pre-move commit>` (it scans `self.` AND `cls.` and reads a revision) and classify the free names against plan 3.2's matrix - that check is what shrank P3.3b by four members, P3.3c by four and P3.3d by one.
7. The remaining P3.3 work is P3.3e (`_derive_investigation_observations`, 2,795 lines) and P3.3f (`_run_investigation_loop`, 3,523), plus the three BLOCKED sub-slices whose unblocking is a LAYER change, not effort. Moving either giant wholesale is a single checkpoint with the whole investigation loop as its failure surface; the design says do the surrounding slices first, which is now done.
8. KEEP THE GUARDS THAT WERE EARNED THE HARD WAY: the extractor's import guard, its arity stop and its derived travelling constants; the contract test's arity pins (`test_no_call_omits_the_host_a_sibling_requires`); and the three probes (`p33-verify.py` for AST identity, `p33c-textdiff.py` for string values + code lines, `p33c-thresholds.py` for surface deltas). Each was added because something slipped past every other gate.
