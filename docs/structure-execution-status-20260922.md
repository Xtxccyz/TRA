...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 62 条，本文件只汇总。

- **被核验的树 = 提交 `9fe2f418788ef2a123da59ab33bde12d39e41e7e`**（该提交的 tree 上跑过四道门禁与全量套件）
- `head_sha` 的语义：**`head_sha` 是被门禁核验的代码提交 `9fe2f418788e`，不是「当前 HEAD」。** 四道门禁、focused 套件与全量套件都在它的 tree 上运行过。**本轮实测到的两处漂移正是这个字段造成的**：先前它记的是提交前的 HEAD，于是文档声称在一个不含本步改动的提交上完成核验；改成「当前 HEAD」后又发现，写下该值的提交本身就会移动 HEAD——**任何文件都无法正确写出「包含自己的那个提交」**。因此这里固定记代码提交，并在每次复验时核对 `src/` 与 `tests/` 是否仍与它一致（本轮实测：`git diff --name-only 9fe2f418788e..HEAD -- src tests` 为空，即逐字节相同）。每一步的回滚点是该步 `rollback_point` 记录的上一个提交。
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

### 读取指引：本文件的哪一部分代表**当前**状态

- **权威内容＝`step_records` + 最新的 `final_state_*` 对象（当前是 `final_state_round_107_p33c2`，由 `authoritative_final_state` 指定）。**
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE (nine packages, 34 moved paths, all seven investigation siblings and all three task modules moved, duplicate list empty, cycle allowlist empty). PHASE 3 IS IN PROGRESS. P3.1 (facade contract) and the whole of P3.2 are COMPLETE. P3.3 is DESIGNED (53 candidates / 8,881 lines / 20-member spine, six slices, 495 lines excluded to P3.4/P3.6 by name) and SEVEN PIECES HAVE MOVED: FOUR SLICES into `investigation/coordinator.py` whose port is SIX members - P3.3b, P3.3a, P3.3c and P3.3d - plus P3.3c(2), the action-proposal group that could not move until the two LAYER ITEMS landed (item 1: `investigation/loop_path.py`, which removed the `investigation -> task` cycle; item 4: `DynamicPlanAction` moved into `contracts.py` and exposed through the model port). `STAYED_FROM_P3_3C` is now EMPTY. `service.py` is down from 29,640 lines at the Phase-3 start to 27,968. The two giant methods are still LAYER-BLOCKED: P3.3e (2,795 lines; 3,041 wholesale, port 6 -> 12) needs the 7 `simulation_adapters` policy names exposed as an allowed emulation interface (layer item 2), and P3.3f (3,523 lines) needs ~34 shared helpers sunk plus the `getsource` assertion at `tests/test_analysis_task_orchestration.py:404` migrated. Also layer-blocked: P3.3b(2) (a helper must move below `report/`, layer item 3) and P3.3d(2) (`methodology` must become a declared shared module, layer item 5). NOT STARTED: P3.4, P3.5, P3.6, P3.7 and Phases 4 and 5. Per-slice detail: `docs/p33-investigation-coordinator-design-20260922.md`, `docs/p33ef-giants-decision-20260922.md`, `docs/p33-layer1-loop-path-module-design-20260922.md`, `docs/p33-layer4-model-action-contract-design-20260922.md` and `docs/p33c2-action-proposal-slice-design-20260922.md`.
...
| phase_3 | IN PROGRESS - P3.1 and P3.2 COMPLETE. P3.3 has FOUR SLICES MOVED (P3.3b/a/c/d; coordinator port = six members) PLUS P3.3c(2) - all four action-proposal members, moved after the two layer items landed - AND TWO LAYER ITEMS DONE (item 1 `investigation/loop_path.py`; item 4 `DynamicPlanAction` in `contracts.py`, exposed by the model port). service.py 29,640 -> 27,968. Remaining, all layer-blocked: P3.3e (3,041 lines wholesale; needs the `simulation_adapters` interface) and P3.3f (3,523 lines; needs ~34 helpers sunk plus one `getsource` migration), plus P3.3b(2) and P3.3d(2). |
...
1. **HARD GATES FOR EVERY SLICE - a slice that is missing any of these is NOT complete, and the next slice does not start until it is (this checklist is the reviewer's recommendation, recorded here because a chat message does not bind a future round): 1. `py scripts/check-slice-tooling.py` - every `scripts/` and `.scratch/` python file parses (a corrupted INSTRUMENT, not the target code, is the failure this exists for; `.scratch` is gitignored so nothing else can see it) plus `--self-check`, and a tamper of the tool a step is about to use. 2. the tool's own can-fail proof, run against a deliberately damaged subject, with every file restored byte-for-byte and hashes compared afterwards. 3. focused tests on the slice's own contract files. 4. the FULL suite. 5. `py scripts/check-deployed-code-hashes.py --strict --import-smoke` (and a rebuild first if `src/` changed), AFTER confirming `git status --porcelain -- src tests` is empty apart from the files this step is about to commit: the gate compares BYTES, so any file something else rewrote makes it fail (MEASURED this round: two tracked files were silently rewritten LF->CRLF and the gate caught it as `differing=1` in all 8 services). 6. failure-set comparison by NODE SET against the baseline, never by count. 7. commit, then confirm `git rev-parse HEAD` equals `git rev-parse origin/main` - and if a push fails, say so in the tracked status rather than claiming sync. On this machine SSH to GitHub is intercepted (banner mismatch, then timeout); the route that works is `git -c url."https://github.com/".insteadOf="git@github.com:" push origin main` through the local proxy. AND: never use 'the file got shorter' or 'more tests pass' as completion evidence - a slice is complete when the CONSUMERS reach the moved implementation through the new home and the old path is blocked by a negative test.**
...
4. DONE and not to be re-opened: P3.3b/a/c/d, P3.3c(2), layer item 1 and layer item 4. THE NEXT STRUCTURAL STEP IS LAYER ITEM 2 (expose the 7 `simulation_adapters` policy names as an allowed emulation interface, or sink the pure policy functions), because it is the only thing that unblocks P3.3e - the 3,041-line `_derive_investigation_observations`, the largest remaining slice. Layer items 3 and 5 unblock P3.3b(2) (97 lines) and P3.3d(2) (271 lines); P3.3f additionally needs ~34 shared helpers sunk and `tests/test_analysis_task_orchestration.py:404`'s `getsource` assertion migrated.
5. BEFORE TRUSTING ANY SLICE INSTRUMENT, RUN IT: `.scratch/p33-extract.py` was found NOT COMPILING at the start of this step (two statements merged onto one line by the PowerShell text mangling this project keeps hitting). The tooling is gitignored, so corruption leaves no `git status` signal - running it is the only detector, and moving these instruments into tracked `scripts/` is now overdue.
6. ANY MOVE MUST DERIVE THE IMPORTS ITS NEW HOME NEEDS and refuse to add them silently. The guard stopped this slice and named exactly the two names the two layer items had made reachable - which is the clearest evidence that those items were the real blockers. Spans must start at the FIRST DECORATOR and carry the comment block above a definition; declarations must preserve the receiver shape (`cls` for classmethods, kept but not forwarded when the moved body needs no host).
7. PINS MUST CHECK ARGUMENTS, NOT ONLY RECEIVERS. A review found that the delegation test constrained only the receiver, so a hostless member's delegation could drop or reorder an argument invisibly - the same class of silent shift the arity pin exists for, in a place the arity pin cannot reach (it only inspects calls whose target takes a host). And when a test that pinned the OLD state is retired by design, its RUNTIME assertions must be carried into the successor rather than dropped with the state.
8. WHEN A TOOL AND ITS VERIFIER ARE WRITTEN TOGETHER, TEST THE VERIFIER AGAINST A DELIBERATELY DAMAGED SUBJECT. Three can-fail scripts now do this for these steps (`layer1-canfail.py`, `layer4-canfail.py`, `p33-canfail.py` plus `p33d-arity-canfail.py`) and all restore every file byte-for-byte.
9. ATTRIBUTE FORMAT DEBT, DO NOT COUNT HUNKS: reflowing shifts line numbers and splits hunks, so a file that was already non-conforming can report more hunks without a single new offending line. `.scratch/format-debt-attribution.py <files> --from <rev>` compares each complaint's removed line against the revision. Moved bodies must NOT be reformatted: byte-identical relocation outranks the formatter.
10. `structure_status=READY` must never imply `capability_status=ACCEPTED`: the Ghidra B3/C3 capability items are untouched by this plan.
