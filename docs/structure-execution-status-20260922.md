...
- **被核验的树 = 提交 `a8c6d0c65a997092258b80503f20d358426c204f`**（该提交的 tree 上跑过四道门禁与全量套件）
...
1. **FIRST: run the two-axis review that P3.6-1 OWES.** The commit `a8c6d0c` was submitted without the parallel Standards/Spec review because the round was consumed by diagnosing and fixing a baseline-external failure; the review is owed and must not be dropped. Fixed point: `f02f4b7` (the previous verified code commit).
2. **THEN P3.5/P3.6-0 (the shared preparation step)**: sink the thirteen `investigation/` contracts plus `ActionCatalog` into `contracts.py` with `X as X` re-exports; route tool execution through `ports.ToolExecutionPort` (executor injected, becomes a host-pin member); sink `TaskLifecycle`/`PackageEntry`; record the `models` and `emulation.policy` decisions. NOTE THE SCOPE: it touches `contracts.py`/`ports.py`/`investigation/**`, which are outside P3.5's and P3.6's allowed files - write the scope note in the step record.
3. **THEN P3.6-2** (`workbench_capabilities`, once `ActionCatalog` is reachable) and **P3.5-2** (17 methods + 2 constants, 6-member pin) once conflict 3 is resolved.
