...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 83 条，本文件只汇总。
...
1. **THEN the owed two-axis review of the P3.7 diff** (fixed point: the commit before this round's records commit; the diff spans 15 test files, +6 tests and the `docs/structure-surface.json` re-record of `test_getsource_count` 39/17 -> 2/0).
