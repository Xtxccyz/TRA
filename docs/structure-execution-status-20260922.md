...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 77 条，本文件只汇总。
...
Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phases 0, 1 and 2 are COMPLETE and P3.1-P3.4 are COMPLETE. **P3.5 IS BLOCKED** (12 of 14 members reach layers the `emulation/` row does not admit; plan conflict 3) and **P3.6 IS DESIGNED AND FEASIBLE** (only 2 of 24 free names are outside the admissible layers, one of them shared with P3.5). Both are unblocked by ONE preparation step, 'P3.5/P3.6-0': sink the `investigation/` contracts into `contracts.py` with `X as X` re-exports, take tool execution through `ports.ToolExecutionPort` with the executor injected, and record the `models` and `emulation.policy` decisions. Then P3.5-2 and P3.6-2, then P3.7 and Phases 4-5. P3.3b(2) and P3.3d(2) remain layer-blocked.
...
| phase_3 | IN PROGRESS - P3.1-P3.4 COMPLETE. **P3.5 BLOCKED** (measured layer conflict; plan conflict 3). **P3.6 DESIGNED AND FEASIBLE** (6 read-only members / 1,040 lines, 12-member pin, only 2 free names outside the admissible layers). ONE shared preparation step ('P3.5/P3.6-0': sink the investigation contracts into contracts.py + route tool execution through ports.ToolExecutionPort + record the models and emulation.policy decisions) unblocks both. Then P3.5-2, P3.6-2, P3.7, Phases 4-5; P3.3b(2)/P3.3d(2) remain layer-blocked. |
...
1. **RUN THE SHARED PREPARATION STEP FIRST ('P3.5/P3.6-0')** - it is the only thing that unblocks P3.5 and it is also what P3.6 needs: (1) sink the thirteen `investigation/` contracts listed in the P3.5 design's section 3 plus `ActionCatalog` into `contracts.py`, with `X as X` re-exports at every old path and zero behaviour change; (2) make the emulation coordinator take tool execution through `ports.ToolExecutionPort` (the concrete `TemporalToolExecutor` is INJECTED and becomes a host-pin member) instead of importing `tools.tool_execution`; (3) sink `TaskLifecycle`/`PackageEntry` into `contracts.py`; (4) record TWO decisions with the decision-(e) mechanism (`recorded_allowed_edges` + `docs/plan-conflict-resolutions-20260922.md`): `emulation.coordinator -> models` and `workbench_query -> emulation.policy` (unless that name is sunk too). NOTE THE SCOPE: this step touches `contracts.py`/`ports.py`/`investigation/**`, which are NOT in P3.5's or P3.6's allowed-file list - write the scope note in the step record rather than widening silently.
2. **THEN P3.6-2** (the ready slice): move the six read-only members (`task_view`, `workbench_domain_view`, `workbench_capabilities`, `workbench_query_evidence`, `model_configuration_view`, `workbench_thread`) plus the two travelling view helpers (`_config_route_view`, `_unique_execution_threads_for_view`) with the 12-member pin. The ten measured write/driver members stay in `service.py`.
3. **THEN P3.5-2** once conflict 3 is resolved (either by the preparation step's hoisting or by an explicit plan decision widening the `emulation/` row): 17 methods + 2 constants, 6-member pin.
4. **HARD GATES FOR EVERY SLICE**: 1. `py scripts/check-slice-tooling.py`; 2. a can-fail proof whose tampers are asserted to land and which restores byte-for-byte, never run concurrently with the suite; 3. `python -m compileall`; 4. the scope gate; 5. `py scripts/check-import-graph.py --strict --policy docs/import-policy.json`; 6. `docker compose config`/`ps`, `--strict --services`, BLOCKED classification, never skipping `ghidra-worker`; 7. focused tests -> full suite -> `scripts/structure_behavior_probe.py --check`; 8. failure-set comparison by NODE SET; 9. the deployment gate after rebuilding BOTH image routes, then `HEAD == origin/main`; 10. the tracked contract test asserts the pin in both directions; 11. the delegation-shape gate (parameters AND `__doc__` equality); 12. for P3.6 additionally the read-only assertion (zero `add`/`delete`/`flush`/`commit`/`merge` in the moved bodies) as a TRACKED test.
