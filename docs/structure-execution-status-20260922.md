...
> 本文件由 `.scratch/structure-status.json` **程序化生成**（`.scratch/render-structure-status.py`）。`.scratch/` 被 gitignore，因此把最终状态在此留一份被跟踪的记录。逐步骤的完整字段（allowed_files / commands / focused_result / full_result / new_failures / import_graph / module_identity / deployment_smoke / behavior_probe_diff / rollback_point / decision）在 `step_records`，共 34 条，本文件只汇总。

- 本文档描述的树经核验于 HEAD `6a221e8444c6302516d5792cd0702fc0686d5abd`（本轮改动在该提交之上，与本文件一并提交）
- **structure_status：`IN_PROGRESS`**
- **capability_status：`UNVERIFIED`**

## 一、为什么不是 READY / ACCEPTED

Plan section P5 grants `structure_status=READY` only after P2-P4 are complete. Phase 0 and Phase 1 are complete. Phase 2 has EIGHT packages in place with 25 moved paths, report/ is COMPLETE, the ONE recorded duplicate implementation is resolved, the first investigation sibling has moved, and BOTH recorded import cycles are retired with an empty allowlist. Phase 2 is still NOT complete: 6 investigation siblings remain (`investigation_ledger.py`, `behavior_catalog.py`, `persist_how.py`, `mechanism_completeness.py`, `mechanism_ready.py`, `semantic_predicates.py`) and so does P2-TK (`task/`). Phases 3-5 have not started. The deployed images DO equal the tree - 109 files x 8 services, missing=0 differing=0 container-only=0, with 37 enumerated smoke imports per container - which satisfies P5.1's mechanical condition, but the plan's own wording makes that necessary and not sufficient.
...
| phase_2 | IN PROGRESS - 8 packages in place; report/ COMPLETE; duplicates NONE recorded and NONE measured; `investigation_protocol.py` has moved into investigation/ and 6 siblings remain, ranked by hazard in `phase_2.investigation_siblings.measured_hazard_ranking`; P2-TK task still to do |
...
109 files x 8 services, missing=0 differing=0 container-only=0, and the import smoke imports 37 enumerated modules in every container (36 before P2-V.2), with all 11 containers running
...
2. next: move the remaining 6 investigation siblings one at a time, each through the SAME copy simulation (`.scratch/p2v2-sim-sibling.py <module>`) plus the sanity check, because the recorded hazards are stale by now. Start with `investigation_ledger.py`; take `semantic_predicates.py` LAST, because `facts/dataflow.py` and `facts/thread_start.py` import it and moving it would create upward edges that need their own analysis.
3. then P2-TK (`task/`)
