...
# SUNK into `facts/investigation_protocol.py` (P3.5-0 M-4) and re-exported with `X as X`, so this path, the root
# `investigation_protocol` shim and every existing importer keep the SAME objects. The PRIVATE helpers moved too but
# are deliberately NOT re-exported: a repo-wide scan shows nothing outside the moved set references them, and
# re-exporting a private name would invite exactly the cross-module private reach this repo's canonical rule bans.
from threat_report_agent.facts.investigation_protocol import (  # noqa: E402
    TEN_QUESTION_SLOTS as TEN_QUESTION_SLOTS,
    is_empty_marker as is_empty_marker,
    empty_slot as empty_slot,
    empty_protocol as empty_protocol,
    function_call_names as function_call_names,
    fill_protocol as fill_protocol,
)
