...
# ``DynamicPlanAction`` is re-exported, not defined here any more: P3.3 layer item 4 moved the canonical class to
# ``threat_report_agent.contracts`` (a pure pydantic contract module the plan's matrix allows ``investigation/`` to
# import) and this path keeps working unchanged. Same object, not a second implementation - pinned by
# ``tests/test_ports.py``.
