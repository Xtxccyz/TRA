...
from threat_report_agent.controlled_emulation import PLACEHOLDER_STATUSES as _EMU_PLACEHOLDER_STATUSES
...
def document_to_markdown(document: dict[str, object]) -> str:
    ...
    for module in document["modules"]:  # type: ignore[index]
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if row.get("type") == "investigation_timeline":
                ...
                lines.append("")
                continue
            if row.get("type") == "decode_result":
                lines.extend(
                    [
                        "### Static Decode Result",
                        f"- evidence_id: `{row.get('evidence_id')}`",
                        f"- artifact_id: `{row.get('artifact_id')}`",
                        f"- verification_status: {_short(row.get('verification_status'))}",
                        f"- source_kind: {_short(row.get('source_kind'))}",
                        f"- function_entry: {_short(row.get('function_entry'))}",
                        f"- rva: {_short(row.get('rva'))}",
                        f"- file_offset: {_short(row.get('file_offset'))}",
                        f"- virtual_address: {_short(row.get('virtual_address'))}",
                        f"- formula: {_short(row.get('formula'))}",
                        f"- length: {_short(row.get('length'))}",
                        f"- printable_ratio: {_short(row.get('printable_ratio'))}",
                        f"- markers: {_short(row.get('markers'))}",
                    ]
                )
                if row.get("decoded_preview"):
                    lines.append(
                        f"- decoded_preview: {_short(_static_safe_text(row.get('decoded_preview')), 600)}"
                    )
                if row.get("decoded_strings"):
                    lines.append(
                        f"- decoded_strings: {_short([_static_safe_text(item) for item in row.get('decoded_strings', [])], 600)}"
                    )
                lines.extend(
                    [
                        f"- consumer_status: {_short(row.get('consumer_status'))}",
                        f"- consumer_candidates: {_short(row.get('consumer_candidates'))}",
                        f"- consumer_evidence_ids: {_short(row.get('consumer_evidence_ids'))}",
                        f"- source_evidence_ids: {_short(row.get('source_evidence_ids'))}",
                        f"- plaintext_recovered: {_short(row.get('plaintext_recovered'))}",
                        "- static_only: true",
                        f"- boundary: {_static_safe_text(row.get('boundary'))}",
                        "",
                    ]
                )
                continue
            if row.get("type") == "static_simulation_prediction":
                lines.extend(
                    [
                        ...
                        f"- evidence_id: `{row.get('evidence_id')}`",
                        f"- function: {_short(_format_function_location(row.get('function'), row.get('entry')))}",
            ...
            if row.get("type") == "api_argument_recovery":
                ...
                visible_arguments, unresolved_slots = _partition_arguments(arguments)
                recovered_count = (
                    len(visible_arguments)
                    if isinstance(arguments, (list, tuple))
                    else int(row.get("recovered_argument_count", 0) or 0)
                )
                lines.extend(
                    [
                        ...
                        f"- function: {_short(_format_function_location(row.get('function'), row.get('function_entry')))}",
                        f"- callsite: {_short(row.get('callsite'))}",
                        f"- consumer: {_short(row.get('consumer'))}",
                        f"- recovered_arguments: {_short(recovered_count)}",
                    ]
                )
                ...
                if unresolved_slots:
                    lines.append(f"- unresolved_argument_slots: {unresolved_slots}")
                reported_count = int(row.get("recovered_argument_count", 0) or 0)
                if reported_count != recovered_count and reported_count:
                    lines.append(f"- recovered_argument_total: {reported_count}")
                ...
                lines.append("")
                continue
            if row.get("type") == "function_semantic_summary":
                calls = row.get("call_sequence", [])
                ...
                summary_recovered_count = (
                    sum(1 for argument in all_call_arguments if _argument_is_meaningful(argument))
                    if all_call_arguments
                    else int(row.get("recovered_argument_count", 0) or 0)
                )
                lines.extend(
                    [
                        f"### Function Semantic Summary: {_short(_format_function_location(row.get('function'), row.get('function_entry')))}",
                        f"- evidence_id: `{row.get('evidence_id')}`",
                        f"- confidence: {_short(row.get('confidence'))}",
                        f"- recovered_arguments: {_short(summary_recovered_count)}",
                    ]
                )
                if calls:
                    lines.append("- ordered_static_calls:")
                    for call in meaningful_calls[:24]:
                        if not isinstance(call, dict):
                            continue
                        api_name, api_category = _semantic_call_label(call)
                        label = f"{api_name} @ {call.get('callsite')} [{api_category}]"
                        lines.append(f"  - {label}")
                        arguments = call.get("arguments", [])
                        visible_arguments, unresolved_slots = _partition_arguments(arguments)
                        for argument in visible_arguments[:8]:
                            lines.append(
                                f"    - arg{argument.get('argument_index')}: {_short(argument.get('value'))} "
                                f"[{argument.get('source_kind') or 'unknown'}]"
                            )
                        if unresolved_slots:
                    ...
                    if omitted_calls:
                        lines.append(
                            f"- omitted_low_information_calls: {omitted_calls} "
                    ...
                    if omitted_internal_calls:
                        ...
                        if internal_clues:
                            lines.append("- unresolved_internal_parameter_clues:")
                            for clue in internal_clues:
                                ...
                                if rendered_arguments:
                                    lines.append(
                                        f"  - callsite {clue.get('callsite') or 'unknown'}: "
                                        f"{rendered_arguments}"
                                    )
                if row.get("conditions"):
                    lines.append("- static_conditions:")
                    for condition in row.get("conditions", [])[:16]:
                        if isinstance(condition, dict):
                            lines.append(
                                f"  - {condition.get('address')}: {_short(condition.get('text'))} "
                                "[outcome UNKNOWN]"
                            )
                if row.get("consumers"):
                    lines.append(f"- downstream_static_consumers: {_short(row.get('consumers'), 1000)}")
                if row.get("unknowns"):
                    lines.append(f"- unknowns: {_short(row.get('unknowns'), 1200)}")
                lines.extend([
                    "- static_only: true",
                    f"- boundary: {_static_safe_text(row.get('boundary'))}",
                    "",
    ...
    markdown = apply_report_display_budget(rendered)
    _stamp_one_round_readiness(document, markdown)
    return markdown
