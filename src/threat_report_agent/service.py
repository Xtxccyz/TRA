...
class AnalysisService:
    ...
    def _config_route_view(self, row: ModelConfiguration, slot: str) -> dict[str, object]:
        ...
        return {
            ...
            "effective_enabled": stored_enabled and self._model_calls_env_enabled,
            "stream": bool(getattr(row, f"{prefix}stream", True)),
            "supports_json_mode": bool(getattr(row, f"{prefix}supports_json_mode", True)),
            "temperature": float(getattr(row, f"{prefix}temperature", 0.0)),
            "top_p": float(getattr(row, f"{prefix}top_p", 1.0)),
    ...
    def model_configuration_view(self) -> dict[str, object]:
        with self.database.session_factory() as session:
            row = session.get(ModelConfiguration, "active")
            if row is None:
                primary_configured = self.settings.primary_model.configured
                return {
                    "source": "user" if primary_configured else "unconfigured",
                    "revision": 0,
                    "enabled": self.settings.model_calls_enabled and primary_configured,
            ...
            return {
                ...
                "enabled": bool(row.enabled) and self._model_calls_env_enabled,
                "stored_enabled": bool(row.enabled),
                "deployment_enabled": self._model_calls_env_enabled,
    ...
    def _unique_execution_threads_for_view(
        ...
        rows = list(
            session.scalars(
                ...
                .where(
                    Evidence.task_id == task_id,
                    Evidence.kind.in_(self._UNIQUE_THREAD_VIEW_KINDS),
                )
                .order_by(
                    case(
                ...
                .limit(self._CATALOG_HOW_SEED_SCAN_LIMIT)
            )
        )
    ...
    def workbench_domain_view(self, task_id: str) -> dict[str, object]:
        ...
        with self.database.session_factory() as session:
            ...
            unique_execution_threads = self._unique_execution_threads_for_view(session, task_id)
            task_payload = {
                "id": task.id,
                "case_id": task.case_id,
                ...
                "server_time": utcnow().isoformat(),
                "failure": self._failure_payload(failure),
                "model_status": self._model_status_payload(failure, model_calls),
