  ...
  factory: (require) => {
    ...
    // Report UI only renders terminal task states: !['SUCCEEDED', 'FAILED', 'CANCELLED'].includes(lifecycle)
      useEffect(() => {
        ...
        const stop = contextStore.subscribe(props, (next) => { if (!alive) return; setSnapshot(next); if (!next.context?.active_task_id) { loadedRef.current = false; setReport(null) } })
        const load = async () => {
          try {
            ...
            if (alive && value) { loadedRef.current = true; setReport(value); setError('') }
          }
      ...
      const coverage = document.analysis_coverage || manifest?.analysis_coverage || {}
      ...
      return h('section', { 'data-threat-view': 'report', style: { fontFamily: 'system-ui, sans-serif', maxWidth: 1080, margin: '0 auto', padding: 18, color: 'var(--dsw-alias-label-primary, #101828)' } },
        h('header', { style: { borderBottom: '1px solid var(--dsw-color-border-subtle, #e4e7ec)', paddingBottom: 14, marginBottom: 16 } }, h('div', { style: { display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'start' } }, h('div', null, h('h2', { style: { margin: 0 } }, '分析报告'), h('p', { style: { margin: '5px 0 0', color: '#667085' } }, `样本：${(document.sample_name || document.display_name || context.attached_artifacts?.[0]?.logical_path || '未知样本')}`)), h('div', null, badge(revision.status || 'READY', 'good'), h('div', { style: { marginTop: 6, fontSize: 12, color: '#667085' } }, `Task ${context.active_task_id}`))), h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 14 } }, metric('Analysis Class', document.analysis_class || context.analysis_class || 'BOUNDED_STATIC_ANALYSIS'), metric('Static Coverage', coverage.overall ?? coverage.coverage ?? '-'), metric('Verified Mechanisms', document.verified_mechanism_count ?? mechanisms.filter((row) => String(row.status).toUpperCase() === 'VERIFIED').length), metric('Unknowns', document.unknown_count ?? '-'), metric('Revision', String(revision.id).slice(0, 12)))),
        h('div', { style: { display: 'flex', gap: 8, marginBottom: 16 } }, h('button', { type: 'button', onClick: () => download('markdown') }, '导出 Markdown'), h('button', { type: 'button', onClick: () => download('docx') }, '导出 DOCX'), h('button', { type: 'button', onClick: () => download('pdf') }, '导出 PDF')),
        ...
        h('h3', null, 'IOC / Indicators'), h('pre', { style: { whiteSpace: 'pre-wrap', background: '#f8fafc', padding: 10, borderRadius: 6 } }, JSON.stringify(document.iocs || document.indicators || [], null, 2)),
        h('h3', null, 'Unknowns / Static Boundaries'), h('p', { style: { whiteSpace: 'pre-wrap' } }, JSON.stringify(document.unknowns || document.limitations || [], null, 2)),
      )
    }
