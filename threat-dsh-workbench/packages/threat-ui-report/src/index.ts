...
export function buildReportViewModel(report: Record<string, unknown>): ReportViewModel {
  const revision = report.revision && typeof report.revision === 'object' ? report.revision as Record<string, unknown> : {}
  return {
    revisionId: revision.id == null ? null : String(revision.id),
    status: String(revision.status ?? 'UNAVAILABLE'),
    markdown: String(revision.markdown ?? report.content ?? ''),
